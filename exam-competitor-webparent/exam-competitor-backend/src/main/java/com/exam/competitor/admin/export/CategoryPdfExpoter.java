...
public class CategoryPdfExpoter  extends AbstactExporter{
	...
	private void writeTableData(PdfPTable table, List<Category> listAll) {
		
		for (Category cat : listAll) {
			table.addCell(String.valueOf(cat.getId()));
			table.addCell(cat.getName().replace("--"," "));
			table.addCell(cat.getAlias());
			table.addCell(cat.getImage());
