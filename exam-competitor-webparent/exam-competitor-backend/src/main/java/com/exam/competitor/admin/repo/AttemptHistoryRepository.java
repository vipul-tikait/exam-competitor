...
public interface AttemptHistoryRepository extends PagingAndSortingRepository<AttemptHistory, Integer> {
	...
	@Query("update AttemptHistory c set c.status = ?2 where c.id = ?1")
	@Modifying
	...
	@Query("select c from AttemptHistory c where CONCAT(c.description) like %?1% and c.user=?2")
	Page<AttemptHistory> findAllOfUser(String keyword, Pageable pageable,User userId);
	
	@Query("select c from AttemptHistory c where c.user=?1")
	Page<AttemptHistory> findAllOfUser(Pageable pageable,User user);
